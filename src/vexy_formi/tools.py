...
class ToolManager:
    ...
    def _load_cache(self) -> Optional[Dict]:
        ...
        try:
            ...
            if (cache_data.get('timestamp', 0) + self._cache_ttl < current_time or
                cache_data.get('env_hash') != current_env_hash):
                return None
    ...
    def _is_tool_available(self, tool: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run([cmd, tool], capture_output=True, timeout=2)
            if result.returncode == 0:
                return True
                
            # Fallback to version check
            result = subprocess.run([tool, '--version'], capture_output=True, timeout=3)
    ...
    def _setup_tool_commands(self):
        ...
        self.minify_commands = {
            ...
            '.html': [
                ('html-minifier-terser', ['html-minifier-terser', '{input}', '-o', '{output}', 
            ...
            '.json': [
                ('jq', ['jq', '-c', '.', '{input}']),  # Compact JSON - will handle output in execute_command
            ],
            '.jsonc': [
    ...
    def _filter_available_commands(self, commands: Dict[str, List[Tuple[str, List[str]]]]) -> Dict[str, List[Tuple[str, List[str]]]]:
    ...
    def get_minify_command(self, file_path: Path) -> Optional[Tuple[str, List[str]]]:
    ...
    def get_format_command(self, file_path: Path) -> Optional[Tuple[str, List[str]]]:
    ...
    def execute_command(self, tool: str, command: List[str], input_file: Path, output_file: Optional[Path] = None) -> Tuple[bool, str]:
        """
    ...
    def _execute_single_command(self, tool: str, command: List[str], input_file: Path, output_file: Optional[Path] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            if output_file:
                cmd = [arg.replace('{input}', str(input_file)).replace('{output}', str(output_file)) 
                      for arg in command]
            else:
                cmd = [arg.replace('{input}', str(input_file)).replace('{output}', str(input_file)) 
                      for arg in command]
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    return False, f"{tool} permission error: {str(e)}"
                    ...
                    if attempt < max_retries and "resource temporarily unavailable" in str(e).lower():
            ...
            return False, f"{tool} unexpected error: {str(e)}"
    ...
    def _quick_tool_check(self, tool: str) -> bool:
        ...
        try:
            ...
            cmd = 'where' if os.name == 'nt' else 'which'
            result = subprocess.run([cmd, tool], capture_output=True, timeout=1)
            return result.returncode == 0
        except Exception:
            return False
    ...
    def execute_command_with_fallback(self, file_path: Path, operation: str) -> Tuple[bool, str, Optional[str]]:
        """
        ...
        for tool, command_template in commands:
            if self.is_available(tool):
                ...
                success, error_msg = self._execute_single_command(tool, command_template, file_path, file_path)
                if success:
                    return True, "", tool
    ...
    def get_tool_info(self) -> Dict[str, Dict[str, any]]:
        ...
        tool_info = {
            ...
            'biome': {'type': 'formatter', 'language': 'Rust', 'formats': ['JS', 'TS', 'JSON', 'CSS']},
            ...
            'html-minifier-terser': {'type': 'minifier', 'language': 'Node.js', 'formats': ['HTML']},
        }
