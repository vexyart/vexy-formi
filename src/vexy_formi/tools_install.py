...
class ToolInstaller:
    ...
    def _detect_architecture(self) -> str:
        ...
        if machine in ['arm64', 'aarch64']:
            ...
            console.print(f"[yellow]Warning: Unknown architecture {machine}, defaulting to x64[/yellow]")
            return 'x64'
    ...
    def install_prerequisites(self) -> bool:
        ...
        if not self.node_available and self.homebrew_available:
            ...
            if self._run_command(['brew', 'install', 'node'], "Node.js installation"):
                ...
                console.print("[yellow]Node.js installation failed, some tools may not be available[/yellow]")
        ...
        if not self.cargo_available:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    console.print("[yellow]Rust installation failed, some tools may not be available[/yellow]")
            except Exception:
                console.print("[yellow]Rust installation failed, some tools may not be available[/yellow]")
        else:
        ...
        with Progress(
            ...
            for tool in tools:
                ...
                if method == 'homebrew' and not self.homebrew_available:
                    ...
                    progress.advance(task)
                    continue
                elif method == 'npm' and not self.node_available:
                    console.print(f"[yellow]Skipping {tool['name']} (npm not available)[/yellow]")
                    progress.advance(task)
                    continue
                elif method == 'cargo' and not self.cargo_available:
                ...
                if self._run_command(tool['cmd'], f"{tool['name']} installation", critical=False):
                    ...
                    if self._check_tool_installed(tool.get('test_cmd', [])):
                        ...
                        console.print(f"[yellow]⚠ {tool['name']} installed but verification failed[/yellow]")
...
'''
        ...
        for filename, config in configs.items():
            try:
                ...
                console.print(f"[yellow]⚠ Failed to create {filename}: {str(e)}[/yellow]")
