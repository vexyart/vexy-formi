...
class VFor:
    ...
    def mini(self, path: str = ".", *, recursive: bool = True, exclude: Optional[List[str]] = None, verbose: bool = False, backup: bool = True, dry_run: bool = False, force: bool = False):
        """
        ...
        with Progress(
            ...
            def progress_callback(result, completed, total):
                nonlocal progress_task, successful_files, failed_files, total_size_before, total_size_after
                
                if progress_task is None:
                    progress_task = progress.add_task(f"Minifying files...", total=total)
                ...
                if verbose:
                    ...
                    if result.success and not dry_run:
                        ...
                        reduction = " (preview)" if dry_run else f" (error)" if result.error_message else ""
            ...
            if not force and not dry_run:
                ...
                if files_to_process:
                    ...
                    summary = safety_checker.get_operation_summary(files_to_process, "minify", dry_run)
    ...
    def fmt(self, path: str = ".", *, recursive: bool = True, exclude: Optional[List[str]] = None, verbose: bool = False, backup: bool = True, dry_run: bool = False, force: bool = False):
        """
        ...
        with Progress(
            ...
            def progress_callback(result, completed, total):
                ...
                if progress_task is None:
                    progress_task = progress.add_task(f"Formatting files...", total=total)
            ...
            if not force and not dry_run:
                ...
                if files_to_process:
                    from .safety import SafetyChecker, get_user_confirmation
                    safety_checker = SafetyChecker()
                    summary = safety_checker.get_operation_summary(files_to_process, "format", dry_run)
    ...
    def tools(self):
        ...
        for tool, info in sorted(tool_info.items(), key=sort_key):
            ...
            formats = ", ".join(info['formats']) if isinstance(info['formats'], list) else info['formats']
        ...
        fast_available = sum(1 for info in tool_info.values() 
    ...
    def ignore(self, action: str = "init", path: str = "."):
        ...
        if action == "init":
            ...
            if self.processor.file_handler.create_vforignore_file(target_path):
                console.print("[green]✓ Created .vforignore file with common exclusion patterns[/green]")
                console.print(f"[dim]Edit {target_path / '.vforignore'} to customize patterns[/dim]")
            else:
            ...
            console.print("  vfor ignore init [path]  # Create .vforignore file with common patterns")
            ...
            console.print("")
            console.print("[dim]Files checked (in order): .gitignore, .vforignore, ../.gitignore, ~/.vforignore_global[/dim]")
    ...
    def _display_benchmark_results(self, benchmark, verbose: bool):
        ...
        if verbose and benchmark.extension_stats:
            ...
            if benchmark.tools_used:
                ...
                for tool, count in sorted(benchmark.tools_used.items(), key=lambda x: x[1], reverse=True):
    ...
    def _display_benchmark_comparison(self, benchmarks):
        ...
        comparison_table.add_row(
            ...
            "Minify" if minify_bench.avg_time_per_file < format_bench.avg_time_per_file else "Format"
        )
    ...
    def perf_check(self, path: str = ".", operation: str = "both", create_baseline: bool = False):
        ...
        if create_baseline:
            ...
            for baseline in baselines:
                console.print(f"  • {baseline.operation}: {baseline.files_per_second:.1f} files/sec")
        else:
    ...
    def _display_results(self, result, operation_name: str, verbose: bool, dry_run: bool = False):
        ...
        if result.successful > 0:
            ...
            files_per_second = result.total_files / result.total_time if result.total_time > 0 else 0
            ...
            console.print(f"Processed {result.successful} files{size_reduction_text} {time_text}{preview_text}")
    ...
    try:
        ...
        try:
            ...
            if "'" in error_str:
                ...
                console.print("[yellow]💡 Try running with elevated privileges or check file permissions[/yellow]")
        except Exception:
            # Fallback to simple error message
            console.print(f"[red]✗ Permission denied: {e}[/red]")
            console.print("[yellow]💡 Check file permissions or run with appropriate privileges[/yellow]")
        sys.exit(1)
    except Exception as e:
        console.print(f"[red]✗ Unexpected error: {e}[/red]")
        console.print(f"[dim]If this persists, please report it at: https://github.com/vexyart/vexy-formi/issues[/dim]")
        sys.exit(1)
