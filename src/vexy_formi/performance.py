...
from .core import FileProcessor, BenchmarkResult
...
class PerformanceBaseline:
    ...
    @classmethod
    def from_benchmark_result(cls, benchmark: BenchmarkResult, version: str = "1.0.3") -> 'PerformanceBaseline':
...
class PerformanceTracker:
    ...
    def __init__(self, baseline_file: Optional[Path] = None):
        self.baseline_file = baseline_file or Path.home() / '.cache' / 'vfor' / 'performance_baselines.json'
    ...
    def save_baseline(self, baseline: PerformanceBaseline) -> None:
        ...
        if len(operation_baselines) >= 10:
            ...
            baselines = [b for b in baselines if not (b.operation == baseline.operation and b.timestamp == operation_baselines[0].timestamp)]
    ...
    def check_regression(self, current: PerformanceBaseline) -> RegressionReport:
        ...
        overall_change = ((current.files_per_second - baseline.files_per_second) / baseline.files_per_second) * 100
    ...
    def create_baseline_from_benchmark(self, path: Path, operation: str = "both") -> List[PerformanceBaseline]:
        ...
        for op in operations:
            # Run benchmark
            if op == "minify":
                result = processor.minify_files(path, recursive=True, create_backup=False, dry_run=True)
            else:
                result = processor.format_files(path, recursive=True, create_backup=False, dry_run=True)
