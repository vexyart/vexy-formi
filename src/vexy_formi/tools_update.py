...
class ToolUpdater:
    ...
    def _run_command(self, cmd: List[str], description: str, critical: bool = True, timeout: int = 300) -> Tuple[bool, str]:
        ...
        try:
            ...
            console.print(f"[red]Error running {description}: {str(e)}[/red]")
            return False, str(e)
    ...
    def update_package_managers(self) -> None:
        ...
        if self.node_available:
            ...
            success, _ = self._run_command(['npm', 'update', '-g', 'npm'], "npm update", critical=False)
            if success:
        ...
        with Progress(
            ...
            for tool in self.tools:
                ...
                if tool.update_method == 'homebrew' and not self.homebrew_available:
                    ...
                    continue
                elif tool.update_method == 'npm' and not self.node_available:
                    console.print(f"[yellow]Skipping {tool.name} (npm not available)[/yellow]")
                    progress.advance(task)
                    continue
                elif tool.update_method == 'cargo' and not self.cargo_available:
                ...
                if success:
                    ...
                    if new_version != tool.current_version:
                        ...
                        console.print(f"[dim]• {tool.name} already up to date ({tool.current_version})[/dim]")
                else:
    ...
    def verify_installations(self) -> None:
        ...
        for tool in self.tools:
            success, _ = self._run_command(tool.check_command, f"{tool.name} verification", critical=False, timeout=30)
            if success:
        ...
        for filename, config_info in config_updates.items():
            ...
            if filepath.exists():
                try:
                    ...
                    console.print(f"[yellow]⚠ Failed to update {filename}: {str(e)}[/yellow]")
            else:
    ...
    def print_summary(self) -> None:
        ...
        for tool in self.failed_updates:
            table.add_row(
                tool.name,
                "✗ Failed",
                tool.current_version or "unknown",
                tool.description
            )
        
        # Add unchanged tools
        unchanged_tools = [t for t in self.tools if t not in self.updated_tools and t not in self.failed_updates]
