...
class SafetyChecker:
    ...
    def analyze_operation(self, files: List[Path], operation: str) -> SafetyReport:
        ...
        for file_path in files:
            try:
                ...
                if size >= self.thresholds.critical_large_file:
                    large_files.append((file_path, size))
                elif size >= self.thresholds.warn_large_file:
                    large_files.append((file_path, size))
        ...
        critical_large_files = [f for f in large_files if f[1] >= self.thresholds.critical_large_file]
        warn_large_files = [f for f in large_files if f[1] >= self.thresholds.warn_large_file and f[1] < self.thresholds.critical_large_file]
        ...
        if operation == "minify" and total_files > 50:
            recommendations.append("Minification creates backup files - ensure sufficient disk space")
        
        if operation == "format" and any(f[1] > 10*1024*1024 for f in large_files):
            recommendations.append("Formatting large files may significantly increase processing time")
    ...
    def get_operation_summary(self, files: List[Path], operation: str, dry_run: bool = False) -> Dict:
